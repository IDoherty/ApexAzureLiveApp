...
func MetricFunc(metricChan <-chan string, outAzureChan chan<- structs.AzureChanStruct, outFileChan2 chan<- string) {
	...
	for {
		...
		//if strings.Compare(azureOut.DevID, "5b3e") == 0 || strings.Compare(azureOut.DevID, "04d3") == 0 || strings.Compare(azureOut.DevID, "147d") == 0 || strings.Compare(azureOut.DevID, "5858") == 0 {
		...
		for x := 0; x < len(maxMetricVals); x++ {
			if strings.Compare(maxMetricVals[x].DevID, azureOut.DevID) == 0 {
				...
				if decodedMetrics.TotalDistance > maxMetricVals[x].TotalDistance {
					maxMetricVals[x].TotalDistance = truncate((decodedMetrics.TotalDistance + maxMetricVals[x].TotalDistance) / 2)
				} else {
